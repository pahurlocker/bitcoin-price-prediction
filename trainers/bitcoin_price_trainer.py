...
class BitcoinPriceModelTrainer(BaseTrain):
    def __init__(self, model, train_data, test_data, config):
        super(BitcoinPriceModelTrainer, self).__init__(model, train_data, test_data, config)
        self.callbacks = []
        self.loss = []
        self.acc = []
        self.val_loss = []
    ...
    def _scale_data(self, X_train, y_train, X_test, y_test):
        ...
        return scaled_train[:,:-1], scaled_train[:, -1].reshape(-1, 1), scaled_test[:,:-1], scaled_test[:, -1].reshape(-1, 1)
    ...
    def train(self):
        ...
        X_scaled_train, y_scaled_train, X_scaled_test, y_scaled_test = self._scale_data(self.train_data[0], self.train_data[1], self.test_data[0], self.test_data[1])
        self.X_train, self.y_train = self._prepare_regression_data(X_scaled_train, y_scaled_train, self.config.trainer.timestep)
        self.X_test, self.y_test = self._prepare_regression_data(X_scaled_test, y_scaled_test, self.config.trainer.timestep)
        if self.config.trainer.model_type_current == 'CNN':
            history = self.model.fit(
                self.X_train.reshape(self.X_train.shape[0], self.X_train.shape[1], self.X_train.shape[2], 1), self.y_train,
            ...
            history = self.model.fit(
                self.X_train.reshape(self.X_train.shape[0], 1, self.X_train.shape[1], self.X_train.shape[2]), self.y_train,
                epochs=self.config.trainer.num_epochs,
                verbose=self.config.trainer.verbose_training,
                batch_size=self.config.trainer.batch_size,
                validation_split=self.config.trainer.validation_split,
            ...
            history = self.model.fit(
                self.X_train.reshape((self.X_train.shape[0], self.X_train.shape[1], 1, 1, self.X_train.shape[2])), self.y_train,
                epochs=self.config.trainer.num_epochs,
                verbose=self.config.trainer.verbose_training,
                batch_size=self.config.trainer.batch_size,
                validation_split=self.config.trainer.validation_split,
        ...
        if self.config.trainer.model_task == 'regression':
            if self.config.trainer.model_type_current == 'CNN':
                X_test_pred = self.model.predict(
                    self.X_test.reshape(self.X_test.shape[0], self.X_test.shape[1], self.X_test.shape[2], 1))
            elif self.config.trainer.model_type_current == 'CNNLSTM':
                X_test_pred = self.model.predict(
                    self.X_test.reshape(self.X_test.shape[0], 1, self.X_test.shape[1], self.X_test.shape[2]))
            elif self.config.trainer.model_type_current == 'ConvLSTM2D_Encoder_Decoder':
                X_test_pred = self.model.predict(
                    self.X_test.reshape((self.X_test.shape[0], self.X_test.shape[1], 1, 1, self.X_test.shape[2])))
            else:
            ...
            if mape.numpy() <= .2:
                self.visualize_prediction(y_test_inverse, X_test_pred_inverse, mape.numpy())
