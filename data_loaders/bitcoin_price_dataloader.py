...
class BitcoinPriceDataLoader(BaseDataLoader):
    def __init__(self, config):
        ...
        df = self._create_technical_indicators(df, (config.trainer.model_task == 'classification'), config.trainer.with_sent)
        
        print(df.head())
        print(df.info())
        self.X_train, self.X_test, self.y_train, self.y_test = train_test_split(
            df.iloc[:, :-1].to_numpy(), df.iloc[:,-1:].to_numpy(), test_size=0.3, shuffle=False, random_state=1)
    ...
    def _create_technical_indicators(self, data, signal=False, with_sent=True):
        ...
        # if with_sent:
        #     data.loc[(data['polarity'] > .6), 'polarity'] = 1
        #     data.loc[(data['polarity'] <= .6), 'polarity'] = 0
