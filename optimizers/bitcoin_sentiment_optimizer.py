...
class BitcoinSentimentOptimizer(BaseOptimizer):
    def __init__(self, config):
        super(BitcoinSentimentOptimizer, self).__init__(config)
        self.project = rubicon.get_or_create_project(
            "Bitcoin Sentiment Analysis v1", description=""
        )

        self.study = optuna.create_study(storage="sqlite:///artifacts/db.sqlite3", 
                study_name="bitcoin-sentiment-{}-{}".format(self.config.optimizer.version,
                datetime.datetime.now().strftime("%Y-%m-%d%H%M%S%z")), direction="maximize")
    ...
    def _objective(self, trial):
        ...
        data_loader = BitcoinSentimentDataLoader(self.config)
        ...
        self.config.trainer.validation_split = 0.2
        ...
        self.config.trainer.num_epochs = trial.suggest_categorical("epochs", [10, 20, 30])
        ...
        if self.config.trainer.model_type_current == "LSTM1":
            ...
            self.config.trainer.embedding_vector_length = trial.suggest_int(
                "embedding_vector_length", 20, 120, 20
            )
            ...
            self.config.trainer.kernal_size = trial.suggest_categorical(
                "kernal_size", [3, 6]
            )
