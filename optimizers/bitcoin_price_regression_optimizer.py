...
class BitcoinPriceRegressionOptimizer(BaseOptimizer):
    def __init__(self, train_data, test_data, config):
        super(BitcoinPriceRegressionOptimizer, self).__init__(train_data, test_data, config)
    ...
    def _objective(self, trial):
        ...
        if self.config.trainer.model_type == '':
            self.config.trainer.model_type_current = trial.suggest_categorical(
                "model_type", ['LSTM', 'CNN', 'GRU', 'CNNLSTM', 'Attention', 'LSTM_Encoder_Decoder', 'TCN', 'ConvLSTM2D_Encoder_Decoder']) 
        else:
        ...
        self.config.trainer.batch_size = trial.suggest_categorical("batch_size", [8, 16, 32])#, 16, 32, 64])
        ...
        if self.config.trainer.model_type_current == 'LSTM':
            ...
            self.config.trainer.units = trial.suggest_categorical("units", [32, 64])
            experiment.log_parameter("units", self.config.trainer.units)
            model = BitcoinPriceEncoderDecoderConvLSTM2DRegressionModel(optimizer, self.config)
        elif self.config.trainer.model_type_current == 'LSTM_Encoder_Decoder':
            self.config.trainer.units = trial.suggest_categorical("units", [32, 64])
            experiment.log_parameter("units", self.config.trainer.units)
            model = BitcoinPriceEncoderDecoderLSTMRegressionModel(optimizer, self.config)
