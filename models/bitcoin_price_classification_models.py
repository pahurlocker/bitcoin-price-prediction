...
class BitcoinPriceEncoderDecoderConvLSTM2DClassificationModel(BaseModel):
    def __init__(self, optimizer, config):
        super(BitcoinPriceEncoderDecoderConvLSTM2DClassificationModel, self).__init__(
            optimizer, config
        )
        self.build_model()
...
class BitcoinPriceAttentionClassificationModel(BaseModel):
    def __init__(self, optimizer, config):
        super(BitcoinPriceAttentionClassificationModel, self).__init__(
            optimizer, config
        )
        self.build_model()
    ...
    def build_model(self):
        ...
        self.model.add(tf.layers.Attention(self.config.trainer.units / 2))
        self.model.add(tf.keras.layers.Dense(1, activation="sigmoid"))
